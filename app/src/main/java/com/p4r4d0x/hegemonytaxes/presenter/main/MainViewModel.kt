...
class MainViewModel @Inject constructor(
    ...
    fun updatePolicy(updatedPolicy: PolicyData) {
        _state.value.policies.find { it.type == updatedPolicy.type }?.let { listPolicy ->
            _state.update { uiState ->
                ...
                uiState.copy(
                    policies = uiState.policies.toMutableList().apply { this[index] = updatedPolicy })
            }
        }
        updateTaxes()
    }
    ...
    fun calculateTaxesResult(data: RoleInputs) {
        _state.update {
            it.copy(resultTaxes = taxRepository.calculateTaxes(it.taxMultiplier, it.incomeTax,it.taxationPolicyState, data))
        }
    }
